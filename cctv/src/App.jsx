...
function App() {
  ...
  const online = cameras.filter((camera) => ['live', 'healthy', 'connected'].includes(camera.status)).length
...
function Page({ page, cameras, alerts, students, attendance, stats, onAddStudent, onDeleteStudent, onCameraAction }) {
  ...
  if (page === 'recordings') return <DataPage title="Recordings" endpoint="/api/recordings" columns={['camera_name', 'event_type', 'status', 'started_at']} />
...
function ReportsPage() { return <><SectionHead eyebrow="REPORT CENTER" title="Reports" /><div className="report-grid">{[['Attendance report', 'Attendance records and daily totals'], ['Safety report', 'Safety events and severity summary'], ['Attendance workbook', 'Printable attendance report'], ['Attendance data export', 'Portable operations data']].map(([title, text]) => <article className="report-card" key={title}><span className="report-icon">▤</span><h3>{title}</h3><p>{text}</p><a className="primary" href={`${API}/download-attendance`}>Download workbook</a></article>)}</div><div className="notice">Exports use the active backend session and current report filters.</div></> }
